...
class Bert(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(bert_feat_dim, 1)

    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor = None) -> torch.Tensor:
        # input_ids is a batch of token ids

        # to use our embedding layer:
        #x = self.embedding(input_ids)
        #bert_outputs = self.bert(inputs_embeds=x)

        # to use BERT's embedding layer, which is optimized for BERT:
        # this gives slightly better results but it's better to only have one set of embeddings in CNN_BERT
        ...
        bert_feat = self.dropout(bert_feat)
        logits = self.fc(bert_feat)
